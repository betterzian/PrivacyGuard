...
> 本文是 `privacyguard/infrastructure/pii/detector/stacks/address.py` 重构 v2 的算法规范与落地分阶段计划。
> 基于 2026-04-10 与用户对齐的 12 点设计点 + 后续修订。
> CLAUDE.md 视当前为开发版，**不做** v1 的兼容层。
...
- HARD clue 走 `BaseStack._build_direct_run`，不进入 component 解析 → 元数据缺失，下游 canonical 退化。
- `deferred_poi` 只是 POI 单独一条特例支路；其他类型（路名、SUBDISTRICT 等）没有"链式吸收"。
- `_TRAILING_ADMIN_TYPES` 仅支持一段尾部逆序（省/市/区），多段逗号场景靠不住。
- "南京浦东中山路" 这类**缺层级的非规范地址**没有 fuzzy 匹配能力，canonical 时直接失配。
- 占位语义没有显式数据结构，靠 `seen_types` 隐式承载，难以扩展。

### 1.2 设计目标

1. **机制统一**：HARD/SOFT、POI/非 POI、正向/逆向走同一套 clue 状态机。
2. **链式吸收泛化**：把 deferred_poi 提升为 "deferred_chain"，覆盖所有 KEY。
...
替代 v1 的 `deferred_poi`，扩展为通用链式缓冲：
...
function run():
    if clue.strength == HARD:
        sub_clues = sub_tokenize(clue.span)        # §5.1
...
function run_with_clues(clues, mode):
    ...
    if state.deferred_chain:
        flush_chain_as_standalone(state)            # §5.3 / §5.4

    if not state.components:
        return None

    fixup_suspected_info(state)                     # §5.6
    components = run_digit_tail(state, clues, cursor)  # 复用 v1
...
- mode='hard_local' 下：
  ...
  - HARD span 至少需要产出一个 component；若 sub_tokenize 后什么都没产出，回退用 span 文本当一个 fallback POI（保留旧 HARD 路径的 recall）。
...
> v1 的 `_left_expand_zh_chars(max_chars=2)` 改为 unit-based。
> v1 的 `_left_expand_en_word` 增加 "find one digit unit" 分支。

### 5.2 链式吸收（设计点 6）

链上的 clue 满足以下条件即可"粘合"：

- 相邻或 gap ≤ 1 unit（**含**正好 1 unit 的情况，设计点 8）
...
### 5.3 VALUE 起栈的 standalone 化（修订版设计点 7）
...
### 5.4 comma-segment + occupancy（设计点 5、8）
...
### 5.6 fixup_suspected_info（设计点 9 修订版）

> 修订点：
>
> 1. 起栈一定从最左 VALUE 开始 → 不需要"road 起栈反向找 admin"。
> 2. fixup 是独立函数，主循环只挂 `raw_chain`，不计算 suspected。
> 3. "南京浦东中山路, city 无, district 有" 的预期：`text="浦东中山", suspected={city:南京}`。
...
沿用 v1：keyed-numbers 优先 → 失败 fallback 到 reversed-subsequence 匹配。
单一变化：BUILDING/DETAIL 仍走 numbers 序列；"NUMBER" 单占位类型也接入 keyed numbers。
...
- `_KEYED_NUMBER_TYPES`：增加 `"number"`（已含），保持。
...
| PR      | 主题                               | 涉及文件                                                                                                            | 风险                                           |
| ------- | -------------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| **PR1** | occupancy 数组 + comma-segment     | `address.py` 主循环；`segment_admit` 抽出                                                                             | 中：替换 `_TRAILING_ADMIN_TYPES` 全部逻辑；回归 v1 测试用例 |
| **PR2** | deferred_chain 泛化（含 §5.1 / §5.2） | `address.py` 链式吸收逻辑；移除 `deferred_poi`                                                                           | 中：影响所有 POI 组合用例                              |
...
| **PR8** | 清理与脚本更新                          | `scripts/debug_address_stack_trace.py`、删除 v1 残留 `_TRAILING_ADMIN_TYPES`、`_ADMIN_DEMOTABLE_AFTER`、`deferred_poi` | 低                                            |
...
- v1 中所有依赖 `_ADMIN_DEMOTABLE_AFTER` 路径的用例（v2 用 chain + suspected 替代）
- v1 中 trailing_admin 单段反序的用例（v2 走 comma-segment）
...
1. **suspected 是否占用 occupancy 槽位？**
  - 最终决议：**不占用**。理由： "上海北京中山路" 出现两份 city 时，后一个city自动降级为普通文本，不进suspected。即相同的admin层级，后一个直接降级为普通的文本。
2. **suspected 的 chain break 方向**
  - 最终决议：从左到右，遇到第一个被降级的 clue → 后续全部降级。
3. **HARD clue 子分词失败的 fallback**
  - 最终决议：把整段当作 fallback POI，保留 v1 的 recall。
4. **POI list 元素之间的顺序**
  - 最终决议：按出现顺序保留。
5. **段内 forward 是否允许跳级**
  - 最终决议：v1 后继图允许 PROVINCE→DISTRICT。v2 沿用。
6. **链穿透中间 KEY 时的 occupancy 行为**
  - 最终决议：穿透的中间 KEY **不写** occupancy。
7. **6-unit gap 是否需要随 segment 重置？**
  - 最终决议：不重置。逗号本身已经把链断开了，新段从下一个 clue 开始，gap 锚点自动跟上。
...
| 维度               | v1                                   | v2                                                      |
| ---------------- | ------------------------------------ | ------------------------------------------------------- |
| HARD clue        | `_build_direct_run` 直通               | sub_tokenize → 主循环                                      |
| 链式吸收             | 仅 POI (`deferred_poi`)               | 所有 KEY (`deferred_chain`)                               |
| 尾部逆序             | `_TRAILING_ADMIN_TYPES` 单段           | comma-segment 多段 + occupancy                            |
| 占位               | `seen_types` 隐式                      | `occupancy: dict` 显式                                    |
| 单占位类型            | 无显式声明                                | `{PROVINCE, CITY, DISTRICT, SUBDISTRICT, ROAD, NUMBER}` |
| 模糊 admin         | `_ADMIN_DEMOTABLE_AFTER`（仅省/市降级为路前缀） | suspected 信息 + fixup pass                               |
| POI 数据           | 单字符串                                 | list                                                    |
| canonical        | strict 逐层 + numbers fallback         | 早退 + suspected fallback + POI 子集                        |
| component commit | 急切（主循环内）                             | 草拟 + fixup pass                                         |
