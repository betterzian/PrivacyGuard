...
@plan_address  要改的地方：1、例 4里，suspect和value比较，不是和rawtext，而是首先和同层级component的value比，在这个例子里，左北京应该是在右中山里去看，是不是子集。这步的意义是看这个左北京是路名还是行政划分，如果能在右road里看到北京两个字，则说明北京是路名，而不是行政划分，但是在路名里看不到北京，说明这两个地址如果路名一样，那么左北京只能是行政名。2、由1可以知道，北京中山路，这可能是一个纯路名，也可能是北京市的一个中山路。所以为了减少误匹配实体，比如把朝阳中山路和江苏省中山路认为是同一个实体，又想让北京中山路和中山路被认为是同一个实体，需要对比较的两个地址都加一个行政区bool值。具体作用是：如果两个地址的行政区（省市区三级）bool都为true，即两个地区都有行政区组件，那么必须要至少有一个相同的行政区层级可以进行值比较，才能在后续比较结果都为真时判定两个地址同一个实体，否则不可以认为是同一实体。但是但凡有一方bool值为false，则只要后续匹配一样即可。3、行政区bool值的计算方式：bool值初始化为false，然后根据以下情况改为true。第一种提交的组件里有行政区层级的key，包括multiadmin组件。第二种某个层级key1里的suspect里的组件key2层级的值，在与对方层级key1组件的值做子集判断时没有命中，则判定该suspect组件不是key1的值，而是对应key2层级，而进入suspect的层级都是管理层，所以key2必定是管理层级，则bool值改为true。

深入思考一下以上问题。对照我的回复，再进行一轮修改，任何你觉得有疑问的地方停下来问我，如果你认为没什么问题，则进行方案优化，落成plan的markdown文档，但是不要以版本号命名，直接用address_plan命名，去除文档中所有版本修改解释，就仿佛这是第一次生成的计划文档。





要改的地方：1、例子（MULTI_ADMIN 与 suspect 混合（OR 链关键场景）

左：北京中山路 → 进入 ROAD 被 KEY 吸收路径：ROAD(value=中山, key=路, suspected=[{level:(P,C), value:北京, key:""}])

右：北京市中山路 → MULTI_ADMIN(北京市, (P,C)) + ROAD(中山, 路)

Step A：admin 比较



左侧没有任何 admin component（"北京"只存在于 suspect）；

枚举右侧 MULTI_ADMIN 的 P / C 解释：每一种下，左侧对应 slot 都是空 → 两侧都没法实配 → inconclusive (None)。

Step B：落到 suspect 3-step OR 链（对 ROAD 这对 peer 做）



Step 1 表面包含：右侧完整 surface 含"北京市" → 左侧 ROAD suspect 的 value "北京" 是其子串 ✓

（已命中，后两步不必触发）

ROAD peer 匹配 → 配合 admin inconclusive → 整体 True。），suspect和value比较，不是和rawtext，而是首先和同层级component的value比，在这个例子里，左北京应该是在右中山里去看，是不是子集。这步的意义是看这个左北京是路名还是行政划分，如果能在右road里看到北京两个字，则说明北京是路名，而不是行政划分，但是在路名里看不到北京，说明这两个地址如果路名一样，那么左北京只能是行政名。2、由1可以知道，北京中山路，这可能是一个纯路名，也可能是北京市的一个中山路。所以为了减少误匹配实体，比如把朝阳中山路和江苏省中山路认为是同一个实体，又想让北京中山路和中山路被认为是同一个实体，需要对比较的两个地址都加一个行政区bool值。具体作用是：如果两个地址的行政区（省市区三级）bool都为true，即两个地区都有行政区组件，那么必须要至少有一个相同的行政区层级可以进行值比较，才能在后续比较结果都为真时判定两个地址同一个实体，否则不可以认为是同一实体。但是但凡有一方bool值为false，则只要后续匹配一样即可。3、行政区bool值的计算方式：bool值初始化为false，然后根据以下情况改为true。第一种提交的组件里有行政区层级的key，包括multiadmin组件。第二种某个层级key1里的suspect里的组件key2层级的值，在与对方层级key1组件的值做子集判断时没有命中，则判定该suspect组件不是key1的值，而是对应key2层级，而进入suspect的层级都是管理层，所以key2必定是管理层级，则bool值改为true。
