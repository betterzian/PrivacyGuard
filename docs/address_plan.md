...
1. 全仓搜索 `component.component_type ==` / `comp_type in` / `_ordered_component_by_type`，列出实际需改动的比较点清单。
...
4. 确认 `_rebuild_component_derived_state` 中 occupancy 重建按 `comp.level` 元组遍历。
5. 确认 `_freeze_key_suspect_from_previous_key` 改写 `entry.key=""` 后，`_fixup_suspected_info` 的 trim 逻辑仍能正确切掉 ROAD.value 前缀。
6. `_SuspectEntry.level` 由 str 改 tuple 后，所有 `entry.level == "..."` 字符串比对处的 call site 都改为 `lvl in entry.level` 或 `entry.level == (lvl,)`。
7. 在 normalized_pii.py 跑一次全仓 grep 列出 7.5 改造所有 call site；确认非 admin 的比较行为等价。
8. 确认 `_ADDRESS_COMPONENT_COMPARE_KEYS` 扩展后 `_same_address` 的 `denom` 计算：MULTI_ADMIN 在 ordered_components 里算一个 entry，不虚占。
9. 确认 has_admin Case 2 在「对侧无同 component_type peer」时按 0.6 语义默认 True（即 `other_component is None` 亦视为 `step1_failed`）。
...
4. **`_should_eager_split_duplicate_dual_admins` 返回 False**："北京北京"（无 KEY，无 anchor）保留为单个 MULTI_ADMIN，第二个静默丢弃。需测试确认。
