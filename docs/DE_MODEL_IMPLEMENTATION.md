...
仓库中已经存在 `application/services/resolver_service.py`，它把 resolver 定位为“约束与回退服务”，并定义了更强的应用层硬约束，例如：
...
### 6.4 bundle / onnx runtime

当前 `DEModelEngine` 已为 `bundle` / `onnx` runtime 类型预留入口，但默认尚未实现完整运行时。
