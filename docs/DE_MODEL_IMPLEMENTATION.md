...
说明：`DecisionFeatureExtractor` → de_model runtime → `ConstraintResolver` 这一细链**仅**在 `decision_mode="de_model"`（`DEModelEngine`）内发生。`label_only` 与 `label_persona_mixed` 同样消费 `DecisionContext` 并调用各自的 `ConstraintResolver`，但不经过 `DecisionFeatureExtractor` 与 `DEModelEngine` 的 torch runtime。
...
**默认 `PrivacyGuard` 与 torch：** 当前默认 `decision_mode` 已收敛为 `label_only`。若显式使用 `decision_mode="de_model"`，则会在 `DEModelEngine` 装配 `runtime_type="torch"` 时导入并使用 PyTorch；若环境中未安装 PyTorch，或未提供 `checkpoint_path`，初始化会直接失败。
...
### 3.6 detector 侧候选辅助：`resolver_service.py`

当前 `application/services/resolver_service.py`（核心类 `CandidateResolverService`）已经收敛为 detector 侧辅助工具，只保留两项能力：

- `build_candidate_id(...)`
- `resolve_candidates(...)`

它不再承担 `DecisionPlan` / `DecisionAction` 的约束、回退或 placeholder 补建。

因此当前默认执行链可以明确理解为：

- candidate 去重与稳定 ID：`CandidateResolverService`
- 动作约束：`ConstraintResolver`
- replacement_text 补全：`ReplacementGenerationService`
...
当前 `DEModelEngine`（`privacyguard/infrastructure/decision/de_model_engine.py`）支持归一化 `runtime_type` 为 `torch` 或 `bundle`。其中：

- `torch`：可执行，且必须提供 `checkpoint_path`
