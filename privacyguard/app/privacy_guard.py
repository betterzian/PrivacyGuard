...
def _load_default_de_model_torch(
    *,
    decision_mode: str,
    decision_engine: DecisionEngine | None,
):
    """默认 de_model 路径前置校验 torch，避免组件装配到一半才失败。"""
    if decision_engine is not None or decision_mode != DEFAULT_DECISION_MODE:
        return None
    try:
        return importlib.import_module("torch")
    except ImportError as exc:
        raise RuntimeError(
            "默认 decision_mode='de_model' 初始化 PrivacyGuard 前需要先安装 torch。"
            "请先安装 train 依赖，例如 `python -m pip install -e '.[train]'`。"
        ) from exc
...
class PrivacyGuard:
    ...
    def __init__(
        ...
        *,
        ...
        self._torch = _load_default_de_model_torch(
