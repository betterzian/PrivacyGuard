...
from training.types import SupervisedTurnLabels, TrainingTurnExample
...
def run_supervised_finetune(config: SupervisedFinetuneConfig) -> SupervisedFinetuneResult:
    ...
    for _epoch in range(config.epochs):
        ...
        for start in range(0, len(order), config.batch_size):
            ...
            batch = batch_builder.build_examples(batch_examples).to(config.device)
            output = model(batch)

            action_targets = _build_action_targets(batch=batch, labels=batch_labels, torch_module=torch, device=config.device)
            loss = F.cross_entropy(
                output.action_logits.reshape(-1, output.action_logits.shape[-1]),
                action_targets.reshape(-1),
                ignore_index=-100,
            )
            persona_targets, persona_target_mask = _build_persona_targets(
                batch=batch,
                labels=batch_labels,
                torch_module=torch,
            ...
            if bool(persona_target_mask.any().item()):
                persona_loss = F.cross_entropy(output.persona_logits[persona_target_mask], persona_targets[persona_target_mask])
                loss = loss + persona_loss
    ...
    torch.save(
        {
            ...
            "training_metadata": {
                "objective": "supervised_behavior_cloning",
    ...
    with Path(path).open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            labels = SupervisedTurnLabels(
                ...
                candidate_actions={
                    candidate_id: ActionType(action_value)
...
def _build_action_targets(*, batch, labels: list[SupervisedTurnLabels], torch_module, device: str):
    targets = torch_module.full(batch.candidate_mask.shape, fill_value=-100, dtype=torch_module.long, device=device)
    action_index = {action: index for index, action in enumerate((ActionType.KEEP, ActionType.GENERICIZE, ActionType.PERSONA_SLOT))}
    for batch_index, label in enumerate(labels):
        candidate_lookup = {candidate_id: index for index, candidate_id in enumerate(batch.candidate_ids[batch_index]) if candidate_id}
        for candidate_id, action in label.candidate_actions.items():
            candidate_index = candidate_lookup.get(candidate_id)
            if candidate_index is None:
                continue
            targets[batch_index, candidate_index] = action_index[action]
    return targets


def _build_persona_targets(*, batch, labels: list[SupervisedTurnLabels], torch_module, device: str):
    targets = torch_module.full((len(labels),), fill_value=-100, dtype=torch_module.long, device=device)
    mask = torch_module.zeros((len(labels),), dtype=torch_module.bool, device=device)
    for batch_index, label in enumerate(labels):
        if not label.target_persona_id:
            continue
        persona_lookup = {persona_id: index for index, persona_id in enumerate(batch.persona_ids[batch_index]) if persona_id}
        persona_index = persona_lookup.get(label.target_persona_id)
        if persona_index is None:
            continue
        targets[batch_index] = persona_index
